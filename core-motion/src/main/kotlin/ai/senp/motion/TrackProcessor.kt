...
class TrackProcessor(private val config: MotionConfig = MotionConfig()) {
    ...
    private fun repair(frames: List<PoseFrame>): List<TrackedFrame> {
        ...
        for (frameIndex in 1 until frames.size) {
            val elapsedMs = frames[frameIndex].timestampMs - frames[frameIndex - 1].timestampMs
            if (elapsedMs >= config.continuityBreakGapMs) {
        ...
        return frames.indices.map { index ->
            TrackedFrame(
                frame = PoseFrame(frames[index].timestampMs, output[index].toList()),
    ...
    private fun smooth(frames: List<TrackedFrame>): List<TrackedFrame> {
        ...
        return frames.mapIndexed { frameIndex, tracked ->
            ...
            tracked.copy(frame = PoseFrame(tracked.frame.timestampMs, smoothed))
        }
    }
