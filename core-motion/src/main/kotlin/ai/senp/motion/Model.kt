...
internal data class Vec3(val x: Double, val y: Double, val z: Double = 0.0) {
...
internal enum class BodySide { LEFT, RIGHT }

internal enum class LandmarkId(val index: Int, val side: BodySide? = null) {
    NOSE(0),
...
internal data class PoseFrame(
    ...
    val inputValidity: FrameValidity = FrameValidity.VALID,
) {
    init {
...
internal enum class FrameValidity {
    VALID,
    REPAIRED,
    DEGRADED,
    BLIND,
...
internal enum class SidePolicy {
    BOTH,
    LEFT_ONLY,
    RIGHT_ONLY,
    BEST_VISIBLE,
...
internal data class QualityWeights(
...
internal data class FrameSignals(
...
internal data class MotionConfig(
